...
class Mega:
    ...
    def _api_request(self, data):
        ...
        # Retry on EAGAIN (-3): temporary server congestion, exponential backoff.
    ...
    def get_public_file_info(self, file_handle, file_key):
        ...
        attribs = decrypt_attr(attribs, k)
        file_size = file_data['s']
        return {'size': file_size, 'name': attribs['n']}
    ...
    def import_public_file(self,
                           ...
                           dest_node=None,
                           dest_name=None):
        # Importing a file into the Cloud Drive
        file_key = base64_to_a32(file_key)
        file_data = self._api_request({
            'a': 'g',
            'g': 1,
        ...
        if dest_node is None:
            # Add to root node if dest_node not given
            if not hasattr(self, 'root_id'):
                self.get_files()
            dest_node = self.root_id
        ...
        encrypt_attribs = base64_url_encode(encrypt_attr(attribs, k))
        key = [
            file_key[0] ^ file_key[4], file_key[1] ^ file_key[5],
            file_key[2] ^ file_key[6], file_key[3] ^ file_key[7], file_key[4],
            file_key[5], file_key[6], file_key[7]
        ]
        encrypted_key = a32_to_base64(encrypt_key(key, self.master_key))
        data = self._api_request({
            'a':
